...
def generate_signature(request_path, body, timestamp, method):
    ...
    print(f"Message for Signature: {message}")  # Debug: Full message
    hmac_key = base64.b64decode(API_SECRET)
    signature = hmac.new(hmac_key, message.encode(), hashlib.sha256)
    generated_signature = base64.b64encode(signature.digest()).decode()
    print(f"Generated Signature: {generated_signature}")  # Debug: Generated signature
...
def test_accounts():
    ...
    print(f"Timestamp Sent: {timestamp}")  # Debug: Timestamp
