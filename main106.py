...
def calculate_amount(action, params, wallet, price):
    """
    Calculate the amount to trade based on parameters and mode.
    Always use the pair amount for calculations, regardless of action.

    Args:
        action (str): "buy" or "sell".
        params (dict): Trading parameters from parameters.json.
        wallet (dict): Current wallet balances.
        price (float): Current market price of the coin.

    Returns:
        float: Amount of coin to trade.
    """
    ...
    if mode == "$":
        # Always calculate using "pair" amount of "coin"
        return fixed_amount / price  # Buy or sell based on pair amount

    elif mode == "%":
        # Use percentage of pair balance
