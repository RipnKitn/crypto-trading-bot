...
def fetch_account_ids():
    try:
        ...
        for account in accounts["accounts"]:
            ...
            account_map[account["name"]] = account["id"]
        return account_map
...
wallet = {
    "coin": client.get_account(account_ids.get(coin))["balance"] if account_ids.get(coin) else 0,
    "pair": client.get_account(account_ids.get(pair))["balance"] if account_ids.get(pair) else 0
}
