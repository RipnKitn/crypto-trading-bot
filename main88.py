...
def execute_trade(action, product_id, percentage=None, flat_amount=None):
    ...
    try:
        if action == "buy":
            trade_crypto(product_id, percentage, flat_amount, action)
        elif action == "sell":
            trade_crypto(product_id, percentage, flat_amount, action)
        else:
...
def trade_crypto(product_id, percentage, flat_amount, action):
    """Handle both buy and sell trades for percentage or flat amounts."""
    base_currency = product_id.split('/')[0] if action == "sell" else product_id.split('/')[1]
    balance = get_balance(base_currency)
    if balance <= 0:
        logging.warning(f"Insufficient {base_currency} balance!")
        return

    trade_size = round((balance * (percentage / 100)), 8) if percentage else flat_amount
    if trade_size < 0.0001:
        logging.warning("Trade size too small!")
        return

    logging.info(f"Placing a {action} order for {trade_size} of {product_id}...")
    ...
    try:
        ...
        logging.info(f"{action.capitalize()} order placed: {response}")
    except Exception as e:
        logging.error(f"{action.capitalize()} order failed: {str(e)}")
...
def webhook():
    ...
    try:
        ...
        if action == "buy":
            execute_trade(
                "buy", product_id, 
                percentage=buy_percentage if mode == "%" else None, 
                flat_amount=buy_flat if mode == "$" else None
            )
        elif action == "sell":
            execute_trade(
                "sell", product_id, 
                percentage=sell_percentage if mode == "%" else None, 
                flat_amount=sell_flat if mode == "$" else None
            )
        else:
