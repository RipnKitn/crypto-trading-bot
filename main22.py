...
def place_trade(symbol, side):
    try:
        base_currency = symbol.split("-")[0]  # e.g., "BTC" from "BTC-USD"
        balance = fetch_balance(base_currency)

        if balance <= 0:
            return {"error": "Insufficient balance"}

        # Set the percentage of the balance you want to trade (e.g., 10%)
        percentage = 0.10  # Trade 10% of your available balance
        ...
        return order  # Return the trade order details (for now, it simulates a successful trade)
    except Exception as e:
