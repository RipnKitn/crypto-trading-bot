...
# New route to fetch supported trading pairs
@app.route("/pairs", methods=["GET"])
def get_supported_pairs():
    try:
        markets = exchange.load_markets()  # Fetch all available markets
        pairs = list(markets.keys())       # Get a list of trading pairs
...
def get_balances():
    try:
        ...
        return {"status": "success", "balances": balances}, 200
    except Exception as e:
        return {"status": "error", "message": str(e)}, 400
