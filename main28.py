...
def fetch_balance(currency):
    try:
        ...
        # Only check the requested currency
        for account in accounts:
            if account["currency"] == currency:
                balance = float(account["available_balance"]["value"])
                print(f"Balance for {currency}: {balance}")  # Debug message
                return balance
        print(f"Currency {currency} not found.")  # Debug message
        return 0.0
...
def place_trade(symbol, side):
    try:
        ...
        print(f"Placing order: {order}")  # Debug message
        return order
    except Exception as e:
